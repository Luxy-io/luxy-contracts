...
describe('ERC1155Luxy', function () {
    ...
    context('with signature functions', function () {
        beforeEach(async function () {
            // fees = [{ account: approved, value: 1 }, { account: anotherApproved, value: 100 }]
            fees = [{ account: operator, value: 5 }]
            await luxy.mint(operator, 10 ,fees,"testURI");

        });
        describe('signature functions', function () {
            it('create signature', async function () {
                expect(await luxy.balanceOf(operator,0)).to.be.equal(10);
		        const signature = await sign(operator, 0, "testURI", fees, luxy.address)

                hash = await luxy.hash1155(operator,0,5)
                await luxy.validate(operator,hash,signature)
            });
