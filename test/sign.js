...
function calculateByMessage(key, message) {
  var hash = util.keccak256(new Buffer.from(message.substring(2), "hex"));
  var signature = util.ecsign(hash, key);
  return {
    v: signature.v,
    r: ("0x" + signature.r.toString("hex")),
...
async function signPersonalMessage(message, account) {
  ...
  var v = util.bufferToInt(new Buffer.from(signature.substr(128, 2), "hex"));
  return {
    v: (v < 27 ? (v + 27) : v),
