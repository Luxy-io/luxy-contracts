...
describe("Tests For Luxy Royalties", function () {
  ...
  beforeEach(async () => {
    ...
    const _royalties = await upgrades.deployProxy(RoyaltiesV1LuxyTest);
  ...
  it("Checking no repeated royalties", async function () {
    ...
    expect(
      royalties.setRoyalties(10, [{ account: creator1, value: 10 }])
    ).to.be.revertedWith("Royalties already set");
  });

  it("Saving shared royalties", async function () {
    //Just to check the contract interface
    const tx = await royalties.setRoyalties(20, [
      { account: creator1, value: 10 },
      { account: creator2, value: 30 },
      { account: creator3, value: 20 },
    ]);
