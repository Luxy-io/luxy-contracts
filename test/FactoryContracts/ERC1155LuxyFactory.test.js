...
describe('ERC1155LuxyFactory', function () {
    ...
    beforeEach(async () => {
        ...
        let tx = await testing.connect(creator1).createToken('Creation1', 'Cri1', '', 1);
        ...
        for (let i = 0; i < tx_receipt.events.length; i++) {
            ...
            if (ev.event == "Create1155LuxyContract") {
                ...
                const response = await testing.connect(creator1).getAddress('Creation1', 'Cri1', '',1);
    ...
    context('factory with minted tokens', function () {
        ...
        describe('check factory contract', function() {
            it('should be equal to creator1 address', async function () {
                expect(await (creator1Contract.address)).to.be.equal(await testing.connect(creator1).getAddress('Creation1', 'Cri1', '',1));
            });
            it ('should be equal to creator1 address', async function () {
